...
export function AccountingPanel() {
  ...
  const loadAccountingData = async () => {
    ...
    try {
      ...
      console.error('Erro na contabilidade:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
          ...
          {['hoje', 'semana', 'mes', 'todas'].map((f) => (
            <button
              ...
              className={`px-6 py-2 rounded-xl text-xs font-black uppercase transition-all ${
                filter === f ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50'
              }`}
            >
              {f}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Vendas</p>
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm border-l-4 border-l-blue-500 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-blue-500 uppercase mb-1">Bruto</p>
            <p className="text-2xl font-black text-gray-800">{formatarMoeda(receitaTotal)}</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm border-l-4 border-l-green-500 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-green-500 uppercase mb-1">Comissões</p>
            <p className="text-2xl font-black text-gray-800">{formatarMoeda(lucroVendedores)}</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm border-l-4 border-l-indigo-600 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-indigo-600 uppercase mb-1">Líquido</p>
            <p className="text-2xl font-black text-gray-800">{formatarMoeda(lucroDono)}</p>
          </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 text-[10px] font-black text-gray-400 uppercase">
              <tr>
                ...
                <th className="px-6 py-4 text-right text-green-600">Comissão</th>
                <th className="px-6 py-4 text-right text-indigo-600">Líquido</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {sales.map((sale) => (
                <tr key={sale.id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 font-black text-gray-700 uppercase text-xs">
                    {sale.seller?.username || 'Sistema'}
                  </td>
                  <td className="px-6 py-4 flex items-center gap-2">
                    <span className="text-xl">{sale.item?.emoji}</span>
                    <span className="text-sm font-bold text-gray-600">{sale.item?.name}</span>
                  </td>
